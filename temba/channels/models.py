...
class Channel(TembaModel):
    ...
    CHANNEL_SETTINGS = {
        ...
        TYPE_TWIML_API: dict(scheme='tel', max_length=1600),
    ...
    def add_gcm_channel(cls, org, user, data):
        ...
        if existing:
            ...
            return Channel.create(org, user, None, Channel.TYPE_GCM, name=org.name, address="gcm-%s" % org.slug[:12], config={'api_key': api_key, 'notification_title': notification_title}, scheme=GCM_SCHEME)
    ...
    def add_twiml_api_channel(cls, org, user, country, address, config, role):
        ...
        return Channel.create(org, user, country, Channel.TYPE_TWIML_API, name=address, address=address, config=config, role=role)
    ...
    def add_whatsapp_channel(cls, org, user, cc, phone, password):
        ...
        with org.lock_on(OrgLock.channels):
            channel = Channel.objects.filter(org=org, channel_type=Channel.TYPE_WHATSAPP, address=config['phone'], is_active=True).first()
            if channel:
                channel.config = json.dumps(config)
                channel.modified_by = user
                ...
                channel = Channel.create(org, user, None, Channel.TYPE_WHATSAPP, name="%s@s.whatsapp.net" % config['phone'],
    ...
    def get_ivr_client(self):
        ...
        if self.channel_type == Channel.TYPE_TWIML_API:
    ...
    def send_twilio_message(cls, channel, msg, text):
        ...
        elif channel.channel_type == Channel.TYPE_TWIML_API:
...
SEND_FUNCTIONS = {Channel.TYPE_AFRICAS_TALKING: Channel.send_africas_talking_message,
                  ...
                  Channel.TYPE_TWIML_API: Channel.send_twilio_message,
